// 6/7 specs passed
//
// Hey Jon, find my comments below marked with `-AZ`
// Nice work. Code is looking very clean.
// In particular check out my comments on magicCipher.
...
function divisibleByFivePairSum(array){
    ...
    for (let i = 0; i < array.length - 1; i++) {
        for (let j = i + 1; j < array.length; j++) {  // good job catching duplicate pairs -AZ
...
function magicCipher(sentence, cipher){
    ...
    for (let i = 0; i < sentence.length; i++) {
        let char = sentence[i];
        for (let key in cipher) { // not need for nested for loops here -AZ
            if (char === key) {
                newSentence += cipher[key];
                // console.log(newSentence);
            }
            else  { // the issue is this else statement, this else will concatenate
                    // the character before we are even certain that the character is NOT
                    // inside the object. -AZ
                newSentence += char
            }
        }
    }
...
// Instead of using a nested loop to check if the key is inside of the object,
// we can use a characteristic of objects to quickly check if a key is inside an object.
//
// For example, if a `key` is NOT INSIDE an `obj`, then `obj[key] === undefined`
//
// -AZ
...
// Excellent use of a single loop that finds the min and the max simultaneously -AZ
